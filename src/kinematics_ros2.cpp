...
vector<VectorXd> Kinematics::oneStepForward(VectorXd& q_in, VectorXd& qd_in, vector<VectorXd> Ld) {
    ...
    integration_time = rclcpp::Clock().now().seconds();
...
//        L_endeffector = L_endeffector + 1e-2 * MatrixXd::Identity(L_endeffector.rows(), L_endeffector.cols());
    ...
    RCLCPP_INFO_THROTTLE(rclcpp::get_logger(), std::chrono::seconds(10), "forward kinematics calculated for %lf s", integration_time);
...
int Kinematics::GetJointIdByName(string joint) {
    ...
    if (joint_index == iDynTree::JOINT_INVALID_INDEX) {
        RCLCPP_INFO_THROTTLE(rclcpp::get_logger(), std::chrono::seconds(5), "joint %s not found in model", joint.c_str());
        return -1;
    }
    return joint_index;
}
